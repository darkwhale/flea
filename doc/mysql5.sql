...
create table `address`(
    `address_id` int(32) not null comment '地址id',
...
create table `user_info`(
    ...
    `user_info_mobile` varchar(16) comment '用户电话',
    `user_qq` varchar(32) comment '用户qq',
    `user_wx` varchar(32) comment '用户微信',
    `user_info_reside_address_id` varchar(128) comment '用户居住地址',
    `user_info_study_address_id` varchar(128) comment '用户学习地址',
    `user_grade` int(4) not null comment '用户年级',
